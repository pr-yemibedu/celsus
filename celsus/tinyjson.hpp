...
/**
 * Added the json_element structure for a slightly different way of retrieving data
 * Updated to compile cleanly with boost 1.38 based on this post: http://www.uwe-arzt.de/node/34
 */ 
...
namespace json
{
  ...
  struct unicodecvt< char >
  {
    static std::string convert(int iUnicode)
    {
      ...
      else if(iUnicode < 0x0800)
      {
        ...
        strString.push_back((char)(0xc0 | ((iUnicode & 0x07c0) >> 6)));
        strString.push_back((char)(0x80 | ((iUnicode & 0x003f) >> 0)));
      }
      else
      {
        // character 0x0800 - 0xffff...

        strString.push_back((char)(0xe0 | ((iUnicode & 0x00f000) >> 12)));
        strString.push_back((char)(0x80 | ((iUnicode & 0x000fc0) >>  6)));
        strString.push_back((char)(0x80 | ((iUnicode & 0x00003f) >>  0)));
      }
  ...
  public:
    typedef std::deque< json_element > JsonList;
    typedef std::map< std::string, json_element> JsonMap;

    json_element() : type(type_invalid) {}
    ~json_element();
    
    template <typename T>
    T safe_get(const T& default_value = T()) const
    {
      if (!is_valid()) {
    ...
    json_element operator[] (const uint32_t i) const;
    const uint32_t length() const;

    json_element find_object(const std::string& object_name) const;
    const bool is_valid() const { return type != type_invalid; }
    static json_element build_from_variant(json::grammar<char>::variant const& var);
    static bool load_from_file(json_element& elem, const char* filename);
  private:
    json_element(const json_type type, const boost::any& obj) : type(type), obj(obj) {}

    static json_element kInvalidElement;
